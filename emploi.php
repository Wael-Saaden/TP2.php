...
<body>
    <form action="emploi.php" method="post">
        ...
        <button type="button" onclick="window.print()">Imprimer</button> <!-- Bouton Imprimer -->
    </form>
    <hr>
    ...
    foreach ($jours as $jour) {
        ...
        foreach ($heures as $heure) {
            if (isset($tab_mat[$jour][$heure])) {
                echo "<td>" . $tab_mat[$jour][$heure] . "<a href='emploi.php?jour=$jour&heure=$heure&action=delmat'><button>Supprimer</button></a></td>";
            } else {
                echo "<td></td>";
            }
        }
